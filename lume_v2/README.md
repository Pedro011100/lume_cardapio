...
Além dos requisitos obrigatórios, o Lume inclui filtro por faixa de preço, ordenação por preço, imagens institucionais, campo de observações, formas de pagamento, cupom de desconto `LUME10`, salvamento do carrinho com localStorage, histórico local de pedidos, modo claro/escuro, confirmação detalhada e geração automática de número do pedido.

O histórico e o tema são armazenados apenas no navegador do cliente. O pedido enviado ao servidor persiste observações, cupom aplicado e valores calculados com validação no backend.
