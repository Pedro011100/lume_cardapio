...
app.post("/api/orders", async (req, res) => {
  const { customerName, tableNumber, paymentMethod, serviceFeeEnabled = true, couponCode = "", notes = "", items } = req.body ?? {};
  ...
  if (typeof couponCode !== "string" || (couponCode.trim() && couponCode.trim().toUpperCase() !== "LUME10")) {
    return res.status(400).json({ error: "Cupom inválido." });
  }
  if (typeof notes !== "string" || notes.length > 500) {
    return res.status(400).json({ error: "As observações devem ter no máximo 500 caracteres." });
  }
  ...
  try {
    const order = await withTransaction(async (database) => {
      ...
      const discount = normalizedCoupon === "LUME10" ? roundMoney(subtotal * 0.1) : 0;
      const afterDiscount = roundMoney(subtotal - discount);
      const serviceFee = serviceFeeEnabled ? roundMoney(afterDiscount * serviceFeeRate) : 0;
      const total = roundMoney(afterDiscount + serviceFee);
      const reference = `CA-${new Date().toISOString().slice(0, 10).replaceAll("-", "")}-${randomUUID().slice(0, 8).toUpperCase()}`;
      const result = await database.run(database.db, "INSERT INTO orders (reference, customer_name, table_number, service_fee_enabled, service_fee_rate, subtotal, service_fee, total, status, payment_method, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'received', ?, ?)", [reference, customerName.trim(), String(table), serviceFeeEnabled ? 1 : 0, serviceFeeRate, subtotal, serviceFee, total, paymentMethod, notes.trim() || null]);
      ...
      return { id: result.lastID, reference, customerName: customerName.trim(), tableNumber: table, paymentMethod, couponCode: normalizedCoupon, discount, notes: notes.trim(), items: priced, subtotal, serviceFee, total };
    });
